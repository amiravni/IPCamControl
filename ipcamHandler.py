...
'''
TODO: 
1. change "min/max" limits according to ref
2. nullify counter at the end of file
3. delete file if no movement detection
...
#    fid.close()    
  ...
  def get_diff_and_score(self,box):
      ...
      if diffScore > MIN_AVG_SCORE:
          ...
          meanDScore = sum(meanD>MIN_AVG_SCORE_SMALL_BOX)
    ...
    while not self.thread_cancelled:
      try:
        if self.img_count == 1:
            ...
            self.fileName_video = MAIN_DIR+self.timeString+'.mkv'
            self.filename_log = MAIN_DIR+self.timeString+'_log.txt'
        ...
        if stream_new_flag == 1:
          ...
          for diffIdx in range(0,len(BOX_WIDTH_HEIGHT)):
              ...
              if diffScore[diffIdx] > MIN_AVG_SCORE and meanDScore[diffIdx] > 0 and meanDScore[diffIdx] < 5:
          ...
          if self.img_count > 1:
              ...
              print 'REF: ',diffScoreRef
              if (diffScore > MIN_AVG_SCORE).any() :
                  currTimeString = self.time2string(time.time())                   
                  print "(",currTimeString ,diffScore,meanDScore,issue,")"
                  fid = open(self.filename_log,'a+') 
                  print >>fid ,currTimeString ,diffScore,meanDScore,issue
                  fid.close()
              self.add_image_to_video()
          if self.img_count == -1:
              self.img_count = 1
